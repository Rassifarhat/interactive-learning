...
// Chapters that have exercise pages implemented
const chaptersWithExercises = [12, 13, 14];
    ...
    <div className="w-full max-w-5xl mx-auto mt-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {chapters.map((chapter) => {
          const hasExercises = chaptersWithExercises.includes(chapter.num);
          ...
          return (
            <div key={chapter.num} className={`flex flex-col border border-${chapter.color}-700 rounded-lg overflow-hidden shadow-lg bg-gray-900 hover:border-${chapter.color}-500 hover:shadow-${chapter.color}-500/40 transition-all duration-300`}>
              ...
              <div className="p-4 flex flex-col space-y-3 flex-grow justify-center">
                ...
                {hasExercises ? (
                  <Link 
                    href={`/csExercisesPageCh${chapter.num}`} 
                    className="block text-center text-yellow-300 hover:text-yellow-100 hover:bg-yellow-700 rounded py-2 px-3 transition-colors duration-200 font-medium border border-transparent hover:border-yellow-500"
                  >
                    Exercises
                  </Link>
                ) : (
                  <span className="block text-center text-gray-500 bg-gray-800 rounded py-2 px-3 font-medium border border-gray-700 cursor-not-allowed">
