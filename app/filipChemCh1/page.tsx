...
import data from '../data/table-of-contents';
...
const ChapterFourteenMindMap: React.FC = () => {
  ...
  // Extract chapter number from pathname (e.g., "/filipChemCh5" -> 5)
    ...
    try {
      ...
      if (savedNodes) {
        ...
        if (Array.isArray(parsedNodes)) {
          ...
          console.log(`[DEBUG] Loaded ${validNodes.length} hidden nodes from localStorage`);
  ...
  useEffect(() => {
    try {
      ...
      console.log(`[DEBUG] Saving ${nodesArray.length} hidden nodes to localStorage`);
  ...
  useEffect(() => {
    ...
    node.append('circle')
      ...
      .attr('r', d => (d.data.id === `chapter-${chapterNumber}` ? 10 : 6))
      .style('cursor', 'pointer')
      .on('mouseover', (e, d) =>
        d3.select(e.currentTarget)
          .attr('r', d.data.id === `chapter-${chapterNumber}` ? 12 : 8)
          .attr('stroke', 'black'))
      .on('mouseout', (e, d) =>
        d3.select(e.currentTarget)
          .attr('r', d.data.id === `chapter-${chapterNumber}` ? 10 : 6)
          .attr('stroke', null));
  ...
  }, [hiddenNodes, chapterNumber]);   // deps unchanged
    ...
    <div className="flex flex-col w-full h-screen bg-gray-800 text-white">
      <div className="p-2 bg-gray-700 text-center sticky top-0 z-10">
        <h1 className="text-xl font-semibold">
          Topic&nbsp;{chapterNumber}: Programming &amp; Data Representation
        </h1>
      </div>
